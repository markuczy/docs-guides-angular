...
:page-aliases: setup/required/configure-share-scopes.adoc
...
See xref:documentation:docs-guides-ui:setup/required/platform-package-compatibility.adoc#package-sharing-convention[Package Sharing Convention] for the list of packages that must be shared (Angular, PrimeNG, NgRx, OneCX, and conditionally `@onecx/angular-utils`) and the versioning rules to follow when declaring them.

[#automatic-package-sharing]
== Automatic Package Sharing

Use `getOneCXSharedRecommendations` from `@onecx/accelerator` instead of hand-writing the `shared` configuration for OneCX packages — it ensures the correct `shareScope` (and other sharing settings) is always set for each dependency.
...
For Angular applications above Angular 21, the shell expects them to be loaded into the `angular_X` share scope corresponding to their Angular version. For applications integrated with older shell versions, the expected share scope is `default`. If no scope is specified for an application, it is loaded into the `default` share scope by default.
