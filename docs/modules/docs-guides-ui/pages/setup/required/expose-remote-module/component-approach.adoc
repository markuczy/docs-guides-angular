...
* `AngularAuthModule` — participates in the shell's authentication flow (see xref:documentation:docs-guides-ui:setup/required/configure-authentication.adoc[Configure Authentication]).
...
`PortalCoreModule` is not needed here: it provides shell-wide UI building blocks (layout, dialog service, breadcrumbs, etc.) that a single Remote Component typically doesn't need. Import it only if the component actually uses one of those building blocks.
